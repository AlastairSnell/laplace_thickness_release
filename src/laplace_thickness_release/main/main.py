...
def build_argparser(*, expert: bool) -> argparse.ArgumentParser:
    ...
    p.add_argument("--heatmap-basic", action="store_true", default=False, help="Use Matplotlib renderer for heatmap instead of PyVista.")
    p.add_argument(
        "--heatmap-screenshot",
        type=Path,
        default=None,
...
def main(argv: list[str] | None = None) -> int:
    ...
    if bool(args.plot) and num_paths > 0:
        if heatmap_mode and heatmap_result is not None and not bool(getattr(args, "heatmap_basic", False)):
            try:

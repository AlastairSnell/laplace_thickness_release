...
def validate_triangles_for_bem(
    ...
    *,
    ...
    dirichlet_tol=1e-6,
    require_watertight=False,  # reserved
):
    issues = []
    ...
    for t_idx, tri in enumerate(triangles):
        ...
        try:
            bc_val_f = float(bc_val)
        except Exception:
            bc_val_f = float("nan")
        if bc_val is None or not np.isfinite(bc_val_f):
        ...
        if bc_type == "dirichlet":
            if min(abs(bc_val_f - dv) for dv in dirichlet_targets) > dirichlet_tol:
                issues.append(f"[tri {t_idx}] Dirichlet bc_value={bc_val_f} not near {dirichlet_targets}")
...
def _triangle_height(area: float) -> float:
    # proxy element scale h = sqrt(area)
...
def _adaptive_integral(
    ...
    if integrand_kind == "S":
        ...
        coarse = _panel_integral_Kp(
            x,
            n_row if n_row is not None else np.zeros(3),
            v0,
            v1,
            v2,
            cfg,
...
def assemble_system(
    ...
    b = np.asarray(bc_val, dtype=np.float64).copy()
...
def evaluate_gradient(
    ...
    for i in range(M):
        ...
        for j in range(N):
            ...
            acc += q[j] * np.asarray(gij, dtype=np.float64)
        grad[i] = acc
    return grad

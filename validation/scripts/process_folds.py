...
and save trimmed mesh pairs + matched boundaries + per-vertex thickness as .npz.

Usage (from repo root, with the package installed or src/ on the path):
    python validation/scripts/process_folds.py
"""
...
def _build_surface_mesh(triangles: list[dict], bc_value: float):
    """
    Extract V (n,3), F (m,3) and sparse adjacency A for one Dirichlet surface.
    Vertex order follows dict insertion order (Python 3.7+).
    """
...
def _reverse_cap_admissible(
    ...
    """
    Return indices of vertices with geodesic distance-to-boundary >= (1 - pct/100) * inradius.
    Mirrors _pick_even_start_points_vertices internals exactly.
    """
...
def _trim_mesh(
    ...
    """
    Keep only faces whose three vertices are all in keep_verts.
    Remove orphan vertices (those not in any kept face), reindex.

    Returns (V_trim, F_trim, old_to_new, used_old_indices).
    """
    ...
    used_verts = np.unique(F_kept)          # old indices actually referenced
    ...
    # every vertex must appear in at least one face
...
def _extract_boundary_loop(F: np.ndarray) -> list[int] | None:
    """
    Extract a single ordered boundary loop using undirected edge counts.

    Algorithm:
      1. For every edge, count how many faces it is a part of.
      2. If it is only part of one face, it is a boundary edge.
      3. The 2 vertices of that edge are boundary vertices.
      4. Order them by walking the connected boundary edges.

    Returns None if boundary is empty or not a single connected loop.
    """
    # Step 1: Count how many faces each undirected edge belongs to
    ...
    # Step 2: Boundary edges = shared by exactly one face
    boundary_edges = [e for e, cnt in edge_count.items() if cnt == 1]

    if not boundary_edges:
        return None

    # Step 3: Collect boundary vertices from boundary edges
    adj: dict[int, list[int]] = {}
    for a, b in boundary_edges:
        adj.setdefault(a, []).append(b)
        adj.setdefault(b, []).append(a)

    # Check that every boundary vertex has exactly 2 neighbors (simple loop)
    for v, neighbors in adj.items():
        if len(neighbors) != 2:
            LOG.warning(
                "Boundary vertex %d has %d neighbors (expected 2) — "
                "not a simple loop.", v, len(neighbors)
            )
            return None

    # Step 4: Walk the loop — follow connected boundary edges in order
    ...
    # Verify we visited all boundary vertices
    if len(loop) != len(adj):
        LOG.warning(
            "Boundary loop covers %d/%d boundary vertices — "
            "multiple loops or disconnected boundary.",
            len(loop), len(adj),
        )
        return None
...
def _geodesic_idw(
    ...
    """Inverse-distance-weighted interpolation with geodesic metric."""
    kd = KDTree(V)
...
def process_patch(vtk_path: Path, out_dir: Path) -> None:
    ...
    # ── Step 5: map seed points → vertex normals ─────────────────────────────────
    ...
    seed_dirs = -1.0 * surface_normals[start_idx_arr]  # point into interior

    # ── Step 6: BEM warm-up (Numba JIT) ─────────────────────────────────────────
    run_single_path(
        0,
        triangles=triangles,
    ...
    # ── Step 7: trace all paths ──────────────────────────────────────────────────
    LOG.info("  Tracing %d paths ...", num_seeds)
    ...
    for i in range(num_seeds):
        idx, _path, length, meta = run_single_path(
            i,
            triangles=triangles,
        ...
        if reasons:
            valid_mask[i] = False
            LOG.debug("  Path %d degenerate (%s).", i, ", ".join(reasons))
    ...
    # ── Step 8: build white mesh & geodesic-IDW thickness ───────────────────────
    ...
    # ── Step 10: trim pial surface (independently) ───────────────────────────────
    ...
    # ── Step 11: extract boundary loops ─────────────────────────────────────────
    #  Each surface gets its complete boundary — no resampling or equalization.
    #  UV alignment is handled downstream after independent Tutte embeddings.
